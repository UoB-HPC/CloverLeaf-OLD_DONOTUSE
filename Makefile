...
build: $(_OBJECTS) $(_MPIOBJECTS) Makefile $(KOKKOS_LINK_DEPENDS) $(KERNELS)
	$(MPI_CC) $(MPI_FLAGS)   $(EXTRA_PATH) $(_OBJECTS) $(_MPIOBJECTS) $(SRCDIR)/clover_leaf.cc $(KOKKOS_LIBS) $(KOKKOS_LDFLAGS) -o clover_leaf

$(OBJDIR)/%.o: $(SRCDIR)/%.cc $(KOKKOS_CPP_DEPENDS)
	$(CC) $(FLAGS) $(MPIINCLUDE) $(KOKKOS_CXXFLAGS) $(KOKKOS_CPPFLAGS) $(EXTRA_INC) -c $< -o $@
	# $(CC) $(FLAGS) $(MPIINCLUDE) $(KOKKOS_CPPFLAGS) $(KOKKOS_CXXFLAGS) $(EXTRA_INC) -MM $< > $(OBJDIR)/$*.d

$(MPIOBJDIR)/%.o: $(SRCDIR)/%.cc $(KOKKOS_CPP_DEPENDS)
	$(MPI_CC) $(MPI_FLAGS) $(KOKKOS_CXXFLAGS) $(KOKKOS_CPPFLAGS) $(EXTRA_INC) -c $< -o $@
	# $(MPI_CC) $(MPI_FLAGS) $(KOKKOS_CPPFLAGS) $(EXTRA_INC) -MM $< > $(MPIOBJDIR)/$*.d


fast: $(_SOURCES) $(_MPISOURCES) Makefile $(KOKKOS_LINK_DEPENDS) $(KERNELS)
	$(MPI_CC) $(MPI_FLAGS) $(KOKKOS_CXXFLAGS) $(KOKKOS_CPPFLAGS) $(EXTRA_PATH) $(_SOURCES) $(_MPISOURCES) $(SRCDIR)/clover_leaf.cc $(KOKKOS_LIBS) $(KOKKOS_LDFLAGS) -o clover_leaf
